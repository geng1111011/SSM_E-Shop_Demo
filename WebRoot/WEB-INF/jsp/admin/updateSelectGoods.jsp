...
<body>
	...
	<c:if test="${allGoods.size() != 0 }">
		<table border="1" bordercolor="PaleGreen">
			...
			<c:forEach items="${allGoods }" var="productInfo">
				<tr onmousemove="changeColor(this)" onmouseout="changeColor1(this)">
					<td>${productInfo.productId }</td>
					<td>${productInfo.productName }</td>
					<td>${productInfo.productPrice }</td>
					<td>${productInfo.productQuantity}</td>
					<td><a href="adminGoods/selectAGoods?id=${productInfo.productId }" target="center">详情</a></td>
					<td><a href="adminGoods/selectAGoods?id=${productInfo.productId }&act=updateAgoods" target="center" >修改</a></td>
				</tr>
			</c:forEach>
			<tr>
